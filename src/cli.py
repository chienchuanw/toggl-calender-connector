...
def display_menu() -> None:
 ...
 _____                _    _____      _                _            
|_   _|__   __ _  __| |  / ____|__ _| | ___ _ __   __| | __ _ _ __ 
  | |/ _ \ / _` |/ _` | | |   / _` | |/ _ \ '_ \ / _` |/ _` | '__|
  | | (_) | (_| | (_| | | |__| (_| | |  __/ | | | (_| | (_| | |   
  |_|\___/ \__, |\__,_|  \____\__,_|_|\___|_| |_|\__,_|\__,_|_|   
          |___/                                                   
    """
    ...
    for item in menu_items:
        # 進階式顯示端口格式
        table.add_row(f"[bright_green]>[/bright_green] {item['key']}", f"[bright_green]{item['name']}[/bright_green]")
...
def handle_custom_sync() -> None:
    ...
    end_date = Prompt.ask(
        "[bright_green]> 結束日期 (YYYY-MM-DD)[/bright_green]", 
        default=start_date
    )
    ...
    continue_process = Confirm.ask(
        "[bright_green]> 確認繼續?[/bright_green]", 
        default=True
    )
...
def sync(
    ...
    console.print(">>> TOGGL CALENDAR CONNECTOR - 同步模式")
    console.print("[bright_green]" + "-" * 70 + "[/bright_green]\n")
    ...
    if days > 0:
        if start_date or end_date:
            console.print("[bright_red]>>> 錯誤：不能同時指定 days 和 start_date/end_date[/bright_red]")
    ...
    # 顯示同步信息
    console.print(f"[bright_green]>>> 準備同步: {calculated_start_date} 到 {end_date_str}[/bright_green]\n")
    
    try:
        client = TogglClient()
        
        with console.status("[bright_green]>>> 正在取得 Toggl 時間條目...[/bright_green]", spinner="dots"):
        ...
        # 顯示找到的時間條目
        console.print(f"[bright_green]>>> 找到 {len(entries)} 個時間條目[/bright_green]\n")
        ...
        # 在預覽模式下，不創建事件
        if preview:
            console.print("[bright_yellow]>>> 預覽模式: 不會創建實際事件[/bright_yellow]")
            return
        ...
        with console.status("[bright_green]>>> 連接到 Google Calendar...[/bright_green]", spinner="dots") as status:
            service = get_calendar_service()
            
            for i, entry in enumerate(entries, 1):
                status.update(f"[bright_green]>>> 正在創建事件 {i}/{len(entries)}...[/bright_green]")
...
def version():
    """顯示版本信息"""
    console.clear()
    console.print("[bright_green]" + "-" * 70)
    console.print(">>> TOGGL CALENDAR CONNECTOR v0.1.0")
    console.print(">>> 將 Toggl 時間條目同步到 Google 日曆的工具")
    console.print("-" + "-" * 70 + "[/bright_green]")
    
    # 添加類似於示例圖像的標語
    console.print("\n[bright_green]>>> 控制你的時間. 掌握你的生產力. 奪回你的一天.[/bright_green]")
    console.print("[bright_green]>>> TOGGL CALENDAR CONNECTOR - 賦予你掌控時間的能力[/bright_green]\n")
